...
export default {
  ...
  methods: {
    addEntry: function(e) {
      axios
        .post("http://localhost:5000/profs/", {
          name: e.name,
          rating: e.rating
        })
        .then(response => {
    ...
    editEntry: function(e) {
      axios
        .put("http://localhost:5000/profs/" + e.index, {
          name: e.name,
          rating: e.rating
        })
        .then(response => {
    ...
    removeEntry: function(e) {
      axios.delete("http://localhost:5000/profs/" + e.index).then(response => {
        this.listOfEntries = response.data;
      });
    }
  },
  mounted() {
    axios.get("http://localhost:5000/profs/").then(response => {
      this.listOfEntries = response.data;
    });
  }
};
